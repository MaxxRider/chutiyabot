...
from robote.heroku.download_wariya import aria_start
from robote.lokal.youtube_dl_button import youtube_dl_call_back
from robote.started.status_message_fn import cancel_message_f
from robote import(MAX_MESSAGE_LENGTH,AUTH_CHANNEL)
 ...
 if "|" in cb_data:
  ...
  if cb_data.startswith("cancel"):
   ...
   if update.from_user.id in AUTH_CHANNEL:
    g_d_list=['.cache','roboteherokulokal','download.conf','robote','robote.log','dht.dat','dht6.dat','filter.txt','Procfile','.robote','.profile.d','start.sh','vendor','venv']
