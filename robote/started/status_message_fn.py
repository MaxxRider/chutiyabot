...
import shutil
import io
from robote import(MAX_MESSAGE_LENGTH,AUTH_CHANNEL,BOT_START_TIME,LOGGER)
from robote.lokal.admin_check import AdminCheck
from robote.heroku.download_wariya import call_apropriate_function,aria_start
from robote.heroku.upload_tege import upload_tege
from robote.lokal.display_progress import(TimeFormatter,humanbytes)
...
async def exec_message_f(client,message):
 if message.from_user.id in AUTH_CHANNEL:
  ...
  if len(OUTPUT)>MAX_MESSAGE_LENGTH:
   ...
   await client.send_document(chat_id=message.chat.id,document="exec.text",caption=cmd,disable_notification=True,reply_to_message_id=reply_to_id)
   os.remove("exec.text")
   await message.delete()
  else:
...
async def upload_document_f(client,message):
 ...
 if message.from_user.id in AUTH_CHANNEL:
  if " " in message.text:
   ...
   recvd_response=await upload_tege(imsegd,local_file_name,message.from_user.id,{})
...
async def eval_message_f(client, message):
    ...
    if len(final_output) > MAX_MESSAGE_LENGTH:
        ...
        await message.reply_document(document="eval.text", caption=cmd, disable_notification=True, reply_to_message_id=reply_to_id )
        os.remove("eval.text")
...
async def upload_log_file(client,message):
 await message.reply_document("robote.log")
