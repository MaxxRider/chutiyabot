...
import os
import time
from robote import(FINISHED_PROGRESS_STR,UN_FINISHED_PROGRESS_STR)
async def progress_for_pyrogram(current,total,ud_type,message,start):
 ...
 if round(diff%10.00)==0 or current==total:
  ...
  progress="[{0}{1}] \n<b>◌Percentage:</b> <code>〘 {2}% 〙</code>\n<b>◌Completed:✓</b> ".format(''.join([FINISHED_PROGRESS_STR for i in range(math.floor(percentage/12))]),''.join([UN_FINISHED_PROGRESS_STR for i in range(12-math.floor(percentage/12))]),round(percentage,2))
