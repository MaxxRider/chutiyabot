...
from robote.lokal.display_progress import progress_for_pyrogram, humanbytes
from robote.lokal.help_Nekmo_ffmpeg import take_screen_shot
from robote.heroku.split_files import split_files
...
async def upload_tege(
    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            splitted_dir = await split_files(local_file_name)
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    #subprocess.Popen(('touch', 'memek.conf'), stdout = subprocess.PIPE)
    with open('memek.conf', 'a', newline="\n", encoding = 'utf-8') as fole:
        fole.write("[DRIVE]\n")
        fole.write(f"{UPLOADER_CONFIG}")
    destination = f'{DESTINATION_FOLDER}'
    if os.path.isfile(file_upload):
        g_au = ['memek', 'copy', '--config=/opt/memek.conf', f'/opt/{file_upload}', 'DRIVE:'f'{destination}', '-v']
        ...
        t_a_m = ['memek', 'lsf', '--config=/opt/memek.conf', '-F', 'i', "--filter-from=/opt/filter.txt", "--files-only", 'DRIVE:'f'{destination}']
        ...
        button = []
        button.append([pyrogram.InlineKeyboardButton(text="☁️ CLOUD LINK ☁️", url=f"{gau_link}")])
        if INDEX_LINK:
            indexurl = f"{INDEX_LINK}/{file_upload}"
            tam_link = requests.utils.requote_uri(indexurl)
            button.append([pyrogram.InlineKeyboardButton(text="⚡ INDEX URL LINK ⚡", url=f"{tam_link}")])
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"🤖: {file_upload} has been Uploaded successfully to your Cloud <a href='tg://user?id={g_id}'>🤒</a>\n📀 Size: {gjay}", disable_web_page_preview=True, reply_markup=button_markup)
        ...
        LOGGER.info(tt)
        t_am = ['memek', 'copy', '--config=/opt/memek.conf', f'/opt/{file_upload}', 'DRIVE:'f'{tt}', '-v']
        ...
        g_a_u = ['memek', 'lsf', '--config=/opt/memek.conf', '-F', 'i', "--filter-from=/opt/filter1.txt", "--dirs-only", 'DRIVE:'f'{destination}']
...
async def upload_single_file(message, local_file_name, caption_str, from_user, edit_media):
    ...
    if UPLOAD_AS_DOC.upper() == 'TRUE':
        ...
        try:
            ...
            if local_file_name.upper().endswith(("MKV", "MP4", "AVI", "M4V", "WEBM")):
